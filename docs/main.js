...
var MainJS = function() {
	console.log("src/MainJS.hx:18:","Hello 'Example Javascript'");
	this.init();
};
...
MainJS.prototype = {
	init: function() {
		...
		window.document.addEventListener("DOMContentLoaded",function(event) {
			$global.console.log("" + model_constants_App.NAME + " Dom ready :: build: " + "2021-09-21 21:42:17" + " ");
			_gthis.initHTML();
			_gthis.loadData();
		});
	}
	...
	,loadData: function() {
		...
		req.onData = function(data) {
			try {
				...
				console.log("src/MainJS.hx:53:",json);
			} catch( _g ) {
				var e = haxe_Exception.caught(_g).unwrap();
				console.log("src/MainJS.hx:55:",e);
			}
		};
		req.onError = function(error) {
			console.log("src/MainJS.hx:59:","error: " + error);
		};
		req.onStatus = function(status) {
			console.log("src/MainJS.hx:62:","status: " + status);
		};
		req.request(false);
	}
};
